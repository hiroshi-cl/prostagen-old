...
public abstract class Converter {
	...
	private String paragraph() throws InconvertibleException {
		...
		while (true) {
			...
			if (line.endsWith("~")) {
				sb.append(inline(line.substring(0, line.length() - 1))).append(paragraphBR()).append('\n');
			} else {
	...
	private String blockPlugin() throws InconvertibleException {
		final Matcher m = Pattern.compile("#ref\\((?:&quot;)?([^,]+?)(?:,([^,]+))*%(?:&quot;)?\\)").matcher(readLine());
		if (m.matches()) {
